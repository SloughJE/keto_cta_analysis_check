...
- less power, compounded noise
- ignores starting plaque and basics like age/sex
...
[Consensus](https://www.fharrell.com/post/errmed/#change-from-baseline) in [biostatistics](https://onlinelibrary.wiley.com/doi/10.1002/sim.2682): use [ANCOVA](https://pubmed.ncbi.nlm.nih.gov/16526009/) [style model](https://pmc.ncbi.nlm.nih.gov/articles/PMC1121605), [follow-up](https://europepmc.org/article/pmc/pmc9557845) ~ baseline + covariates for pre/post analyses.
...
1. Figures, data, and tests don’t match

::: {.fragment}

2. Key analyses not reported

:::
::: {.fragment}

3. Modeling choices systematically weaken evidence for an association

:::
::: {.fragment}

4. Analytic design flaws across multiple analyses

:::

::: {.fragment}

5. ANCOVA is the clinically relevant model
...
Figures 2 (D to F) Change in ApoB, baseline ApoB, and baseline CAC vs **NCPV**. Actually TPS.

R2, actually adjusted R2

Table 1 Ketogenic diet duration (d) mean: 1,642.7 (4.5 years)
Table 3 caption; "LDL-C exposure while on a ketogenic diet (mean 5.7 years);" 

LDL exposure:
Abstract: total LDL-C exposure:   median 1,302 days, Q1-Q3: 984 - 1,754 days
Table 1: Ketogenic diet duration: median 1,427 days, Q1-Q3: 1,002-1,938 days

also exposure should be in mg/dL·days (not days).

Total cholesterol (mg/dL) median: 338 IQR (301-337) cannot be right.

Baseline Characteristics Table. 
Why No Follow-up Characteristics table?

multiple: "changes in baseline levels". Baseline levels don't change.

Direct quote from CENTRAL ILLUSTRATION:
> "Neither total exposure nor changes in baseline levels of ApoB and mg/dL were associated with changes in plaque. Conversely, baseline plaque but ApoB was not associated with plaque progression."


"using R version 4.0.3 (R Core Team [2020], R Foundation for Statistical Computing, with the last available version for September 2024)." Does that mean they used version 4.0.3 from 2020 or version 4.4.2 from Sept 2024?
...
**This isn’t percent change.**

They computed a **ratio of median change to baseline median:** $\dfrac{\mathrm{median}(ΔNCPV)}{\mathrm{median}(\mathrm{NCPV_{bl}})}\times 100\%$  
...
**Standard percent change:** $\text{median}\!\left(\frac{ΔNCPV}{NCPV_{bl}} \times 100\%\right)$
...
Their "percent change" metric is **mislabelled**, **nonstandard** and not well explained.
...
They took the **group median change** and divided it by the **group baseline median**:

- **NCPV:** \( \dfrac{18.9\ \text{mm}^3}{44\ \text{mm}^3} \approx 43\% \)
- **PAV:** \( \dfrac{0.8\ \text{pp}}{1.6\%} \approx 50\% \)

They then wrote: “Compared to baseline, these represent a **43%** and **50%** change,” and elsewhere referred to “percentual changes,” while the actual reported deltas are **absolute** (mm³) and **percentage-point** (pp), not per-person % changes.

---

## Why this isn’t a “percent change”

- **Wrong statistic:**  
  \( \dfrac{\mathrm{median}(\Delta)}{\mathrm{median}(\text{BL})} \;\ne\; \mathrm{median}\!\left(\dfrac{\Delta}{\text{BL}}\right) \).  
  With skewed data or low baselines, these can differ a lot.

- **Unit mixing for PAV:**  
  PAV’s “0.8%” is a **percentage-point** change. Turning that into “50%” by dividing by a **baseline %** mixes **pp** with **percent**.

- **Not comparable across studies:**  
  Most CCTA papers report **per-person** percent change (median [IQR]) and/or **annualized** pp or mm³ change. A **ratio-of-medians** is rarely (if ever) reported, so it doesn’t line up with others.

- **Narrative mismatch:**  
  They warn that “low baselines magnify % change,” but they never report **per-person % changes**—only this group-level ratio and absolute/pp deltas.

---

## What they should have shown (and what you can show)

- **Per-person % change:**  
  \( \dfrac{\text{FU} - \text{BL}}{\text{BL}} \times 100\% \) → summarize with **median [IQR]**.

- **Percentage-point change for PAV:**  
  \( \text{PAV}_{\text{FU}} - \text{PAV}_{\text{BL}} \) (**pp**), optionally annualized.

- **Baseline & follow-up medians (IQR):**  
  Show both visits so readers can see the actual 1-year levels.

---

## Slide-ready text you can paste

> **Not a real “% change.”** The paper’s “43% (NCPV) and 50% (PAV)” come from a **ratio-of-medians**: \((\mathrm{median\ change})/(\mathrm{median\ baseline})\), not the **median per-person % change**. For PAV they also mix **percentage-point** change (0.8 pp) with “percent,” which is misleading. This statistic isn’t standard and isn’t comparable to studies that report **per-participant % change** or **annualized** pp/mm³ change.

---

## Tiny formula callout (for the slide)

- **What they did:**  
  \( \%\_{\text{(their)}} \;=\; \dfrac{\mathrm{median}(\Delta)}{\mathrm{median}(\text{BL})} \times 100\% \)

- **What readers expect:**  
  \( \%\_{\text{change}} \;=\; \mathrm{median}\!\left( \dfrac{\text{FU} - \text{BL}}{\text{BL}} \times 100\% \right) \)
