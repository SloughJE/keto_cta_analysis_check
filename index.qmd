...
- baseline imbalance and shared correlates can inflate, reduce, or even reverse associations
...
`NCPV_follow-up ~ NCPV_basline + ApoB`
...
- **Baseline R² ≈ 0.95:** 95% of follow-up NCPV variance is explained by **baseline NCPV**.
- **Residual R² after baseline: 5%**  (for risk factors + measurement error + noise)
- **ApoB effect:** ΔR² ≈ 0.03 ⇒ Partial R² (ApoB | baseline) = 0.03 / 0.05 = 0.60  
- ApoB explains 60% of the variance remaining after baseline.
...
- R² must increase when adding predictors; the key is how much of the **leftover explainable residual** variance is explained:  
  residual after Baseline+age+sex is 1−0.50=0.50; ApoB adds 0.02 → partial R² = 0.02/0.50=0.04.
- Messaging: “ApoB adds **unique** information about progression beyond Baseline; ‘small’ ΔR² on the total scale understates its share of the explainable residual.”

- **Everything else:** \(1 - 0.95 - 0.03 = 0.02\) → ~**2%** left  
...
4. Despite the claim of being "corroborated", model assumptions were violated. Many models were misspecified.
